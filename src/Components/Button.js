...
const Button = ({outline, name, func, alternate}) => {
    if(alternate && outline){
        return (
            <button onClick={()=>func()}  className="w-1/2 border-2 border-yellow-300 py-2 md:w-1/3 rounded-lg text-yellow-300  text-center bg-black ">{name}</button>
        )
    }
    if(alternate && !outline){
        return (
            <button onClick={()=>func()}  className=" text-black w-1/2 border-none outline-none py-2 md:w-1/3 rounded-lg  text-center bg-yellow-300">{name}</button>
        )
    }
    if (!alternate && !outline){
        return (
